/**
 * uceditor.js for Drupal7 by M
 */

(function ($) {

  Drupal.uceditor = Drupal.uceditor || {};
  
  Drupal.uceditor.render = function(element, setting) {
    if(Drupal.settings.ckeditor 
        && Drupal.settings.ckeditor.input_formats
        && Drupal.settings.ckeditor.input_formats[setting])
    return ;
    var editor = new baidu.editor.ui.Editor(Drupal.settings.uceditor.input_formats[setting]);
    editor.render(element);
  }

  Drupal.uceditor.admin_profile_form = function() {
    var barkey = '|';
    var barstr = '<div id="edui-toolbar" class="edui-toolbar">';    
    for(var k in Drupal.uceditor.default_toolbars) {
      barkey = Drupal.uceditor.default_toolbars[k];
      if ( barkey != '|' )
    ...
    Drupal.uceditor.admin_profile_form_init($('#edit-toolbars').val());
  }
  
  Drupal.uceditor.admin_profile_form_init = function(v) {
    var barstr = v.split(',');
    var icodiv;
  ...
  Drupal.uceditor.default_toolbars = ['fullscreen', 'source', '|', 'undo', 'redo', '|',
